...
def prowler(check):
    ...
    file_name = check
    with open(f'tools/prowler/{check}.csv', 'w') as output:
        subprocess.call(['./prowler', '-M', 'csv', '-c', check], stdout=output, cwd='tools/prowler')

    csvfile = open(f'tools/prowler/{check}.csv', 'r')
    jsonfile = open(f'tools/prowler/{check}.json', 'w')
...
def multi_threaded_prowler():
    ...
    for check in checks:
        dict = {}
        data = []
        with open(f'tools/prowler/{check}.json', 'r') as f:
            for line in f:
                j = json.loads(line)
                dict['check'] = j['check']
                new_dict = {
...
def scout2():
    ...
    subprocess.call(
        [
            ...
            '--no-browser',
            '--report-dir',
            f'../../reports/AWS/aws_audit/{account_name}/{timestmp}/{file_name}',
        ],
        cwd='tools/Scout2',
    )
...
def audit_aws_certs():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/certs', 'w') as output:
        subprocess.call(['python', './scripts/audit_aws_certs.py'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/certs'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,CERT_AUDIT,No information is available\n")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/certs')
...
def audit_aws_cf():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/cloud_formation', 'w') as output:
        subprocess.call(['./scripts/audit_aws_cloud_formation.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/cloud_formation'
            ).st_size
            == 0
        ):
...
def audit_aws_config():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/aws_config', 'w') as output:
        subprocess.call(['./scripts/audit_aws_config.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/aws_config'
            ).st_size
            == 0
        ):
...
def audit_aws_dns():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/dns', 'w') as output:
        subprocess.call(['./scripts/audit_aws_dns.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/dns'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,DNS_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/dns')
...
def audit_aws_ec():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec', 'w') as output:
        subprocess.call(['./scripts/audit_aws_ec.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,ELASTIC_CACHE_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec')
...
def audit_aws_ec2():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec2', 'w') as output:
        subprocess.call(['./scripts/audit_aws_ec2.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec2'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,EC2_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ec2')
...
def audit_aws_elb():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/elb', 'w') as output:
        subprocess.call(['./scripts/audit_aws_elb.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/elb'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,ELB_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/elb')
...
def audit_aws_es():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/es', 'w') as output:
        subprocess.call(['./scripts/audit_aws_es.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/es'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,ELASTIC_SEARCH_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/es')
...
def audit_aws_keys():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/keys', 'w') as output:
        subprocess.call(['./scripts/audit_aws_keys.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/keys'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,AWS_KEY_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/keys')
...
def audit_aws_rds():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/rds', 'w') as output:
        subprocess.call(['./scripts/audit_aws_rds.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/rds'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,RDS_AUDIT,No information is available")
    print ("AWS RDS Audit Done ")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/rds')
    return 0
...
def audit_aws_redshift():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/redshift', 'w') as output:
        subprocess.call(['./scripts/audit_aws_redshift.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/redshift'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,REDSHIFT_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/redshift')
...
def audit_aws_ses():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ses', 'w') as output:
        subprocess.call(['./scripts/audit_aws_ses.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ses'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,SES_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/ses')
...
def audit_aws_cdn():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/cdn', 'w') as output:
        subprocess.call(['./scripts/aws_cdn_audit.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/cdn'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,CDN_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/cdn')
...
def audit_aws_sns():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/sns', 'w') as output:
        subprocess.call(['./scripts/audit_aws_sns.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/sns'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,SNS_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/sns')
...
def audit_aws_vpcs():
    ...
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/vpc', 'w') as output:
        subprocess.call(['./scripts/audit_aws_vpcs.sh'], stdout=output)
        if (
            os.stat(
                f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/vpc'
            ).st_size
            == 0
        ):
            output.write("default,account,null,null,INFO,scored,null,VPC_AUDIT,No information is available")
    csv_to_json(f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta/vpc')
    print ("AWS VPC Audit Done ")
    return 0

def trusted_advisor():
    with open(f'reports/AWS/aws_audit/{account_name}/{timestmp}/final_report/trusted', 'w') as f:
    ...
    csv_to_json(
        f'reports/AWS/aws_audit/{account_name}/{timestmp}/final_report/trusted'
    )
...
def aws_audit():
    ...
    subprocess.call(
        [
            'mkdir',
            '-p',
            f'reports/AWS/aws_audit/{account_name}/{timestmp}/final_report',
        ]
    )
    subprocess.call(
        [
            'mkdir',
            '-p',
            f'reports/AWS/aws_audit/{account_name}/{timestmp}/delta',
        ]
    )
