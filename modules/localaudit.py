...
def local_audit(audit_ip, user_name, pem_file, password):
    ...
    if type == 'PUBLIC':
        operating_sys = subprocess.check_output(
            [
                'aws',
                'ec2',
                'describe-instances',
                ...
                f'{default_region}',
                '--filters',
                f'Name=ip-address,Values={audit_ip}',
                '--query',
                'Reservations[*].Instances[*].[Platform]',
                '--output',
                'text',
            ]
        ).strip()
        ...
        operating_sys = subprocess.check_output(
            [
                ...
                f'{default_region}',
                '--filters',
                f'Name=network-interface.addresses.private-ip-address,Values={audit_ip}',
                '--query',
                'Reservations[*].Instances[*].[Platform]',
                '--output',
                'text',
    ...
    if operating_sys == 'windows':
        ...
        elif audit_ip and user_name and not (password):
